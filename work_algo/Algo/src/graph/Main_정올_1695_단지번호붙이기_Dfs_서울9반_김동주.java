...
public class Main_정올_1695_단지번호붙이기_Dfs_서울9반_김동주 {
	...
	public static void dfsr(int i, int j, int cn) { //재귀호출 사용하는 방법
		//cnt =0;
		//visit[i][j]=true;
		//여기서 초기및return조건 체크!!
		map[i][j]=cn;
		...
		for(int d=0; d<4; d++) { 
			...
			if(0<=ni && ni<N && 0<=nj && nj<N && map[ni][nj]==1) {// && visit[ni][nj]==false) { //범위체크 중요!! 범위체크 먼저, maze체크 다음
				dfsr(ni,nj,cn); //리턴이 있을 경우 받아서 쭉넘겨줘야함!!!!if(dfsr(ni,nj,cn)==1) return 1;
			}
		}
	}
